...
class _MapScreenState extends State<MapScreen> {
  ...
  _addPolyLine() {
   ...
   _getPolyline() async {
    var origin = PointLatLng(widget.userLat.toDouble(), widget.userLong.toDouble());
    var destination=PointLatLng(widget.marketLat.toDouble(), widget.marketLong.toDouble());
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        googleApiKey: "AIzaSyDuhz7OV0z0rykuvN-fdylPzPBkAipucX4",
        request: PolylineRequest(origin: origin, destination: destination, mode: TravelMode.driving, wayPoints:[PolylineWayPoint(location: widget.address)] ));
